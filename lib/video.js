...
/** @inheritDoc */
zb.device.platforms.tvip.Video.prototype.getAspectRatio = function() {
	//todo
};


/** @inheritDoc */
zb.device.platforms.tvip.Video.prototype.setAspectRatio = function(ratio) {
	//todo
};


/** @inheritDoc */
zb.device.platforms.tvip.Video.prototype.setDisplayArea = function(x, y, width, height) {
	var coefficient = this._coefficient;
	this._displayAreaStateChanged = this._isDisplayAreaChanged.apply(this, arguments);

	if (this._displayAreaStateChanged) {
		this._savePreviousVideoRect();

		this._saveNewVideoRect.apply(this, arguments);
		this._tvipPlayer
			.setVideoWindow(x * coefficient, y * coefficient, width * coefficient, height * coefficient, false);
	}

	zb.console.debug(
		'zb.device.platforms.tvip.Video::SetDisplayArea(x, y, w, h)',
		x, y, width, height
	);
};
...
/** @inheritDoc */
zb.device.platforms.tvip.Video.prototype.setDisplayFullScreen = function(state) {
	if (state) {
		this.setDisplayArea(
			this._videoContainerRect.x,
			this._videoContainerRect.y,
			this._videoContainerRect.width,
			this._videoContainerRect.height
		);
	} else if (this.isDisplayFullScreen()) {
		this.setDisplayArea(
			this._previousVideoRect.x,
			this._previousVideoRect.y,
			this._previousVideoRect.width,
			this._previousVideoRect.height
		);
	}
};
...
/** @inheritDoc */
zb.device.platforms.tvip.Video.prototype.isDisplayFullScreen = function() {
	return (
		this._displayAreaRect.x === this._videoContainerRect.x &&
		this._displayAreaRect.y === this._videoContainerRect.y &&
		this._displayAreaRect.width === this._videoContainerRect.width &&
		this._displayAreaRect.height === this._videoContainerRect.height
	);
};
 ...
 * @protected
 */
zb.device.platforms.tvip.Video.prototype._isDisplayAreaChanged = function(x, y, width, height) {
	this._displayAreaRect = this._displayAreaRect || this._videoContainerRect;

	return this._displayAreaRect.x !== x ||
		this._displayAreaRect.y !== y ||
		this._displayAreaRect.width !== width ||
		this._displayAreaRect.height !== height;
};
 ...
 * @protected
 */
zb.device.platforms.tvip.Video.prototype._savePreviousVideoRect = function() {
	this._previousVideoRect = {
		x: this._displayAreaRect.x,
		y: this._displayAreaRect.y,
		width: this._displayAreaRect.width,
		height: this._displayAreaRect.height
	};
};
 ...
 * @protected
 */
zb.device.platforms.tvip.Video.prototype._saveNewVideoRect = function(x, y, width, height) {
	this._displayAreaRect = {
		x: x,
		y: y,
		width: width,
		height: height
 ...
 * @protected
 */
zb.device.platforms.tvip.Video.prototype._initDisplayArea = function() {
	var mode = this._tvipStb.getDisplayMode();
	var container = app.getVideoContainer();

	this._coefficient = parseInt(mode.replace(/\D+/, ''), 10)/container.offsetHeight;
	this._videoContainerRect = {
		x: container.offsetLeft,
		y: container.offsetTop,
		width: container.offsetWidth,
		height: container.offsetHeight
	};
	this.setDisplayFullScreen(true);
};
 ...
 * @protected
 */
zb.device.platforms.tvip.Video.prototype._displayAreaRect;
 ...
 * @protected
 */
zb.device.platforms.tvip.Video.prototype._previousVideoRect;


/**
 * NOTE: платформа может проигрывать видео в различных форматах, поэтому используем коэффициент
 * @type {number}
 * @protected
 */
zb.device.platforms.tvip.Video.prototype._coefficient;
 ...
 * @protected
 */
zb.device.platforms.tvip.Video.prototype._videoContainerRect;
 ...
 * @protected
 */
zb.device.platforms.tvip.Video.prototype._displayAreaStateChanged;
