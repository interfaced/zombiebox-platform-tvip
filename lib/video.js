...
/**
 * @param {zb.device.platforms.tvip.Video.State} oldState
 * @param {zb.device.platforms.tvip.Video.State} newState
 * @private
 */
...
zb.device.platforms.tvip.Video.prototype._startUpdatePosition = function() {
	...
	var isBuffering = this._checkBuffering(currentPosition);//todo
	if (!isBuffering && (this._state === zb.device.IVideo.State.PLAYING || this._state === zb.device.IVideo.State.SEEKING)) {
...
zb.device.platforms.tvip.Video.prototype._checkBuffering = function (currentPosition) {
	if (currentPosition === this._lastPosition) {
		if (this._state !== zb.device.platforms.tvip.Video.State.BUFFERING) {
			this._setState(zb.device.platforms.tvip.Video.State.BUFFERING);
		}
		return true;
	} else {
