...
zb.device.platforms.tvip.ViewPort = class extends zb.device.ViewPort {
	...
	_initAspectRatioMap() {
		...
		const createAspectRatio = (proportion, transferring) => new zb.device.aspectRatio.AspectRatio(proportion, transferring);

		this._aspectRatioMap[TVIPRatio.NORMAL] = createAspectRatio(Proportion.Common.AUTO, Transferring.AUTO);
		this._aspectRatioMap[TVIPRatio.FULL] = createAspectRatio(Proportion.Common.AUTO, Transferring.STRETCH);
		this._aspectRatioMap[TVIPRatio.ZOOM] = createAspectRatio(Proportion.Common.AUTO, Transferring.CROP);
	}
