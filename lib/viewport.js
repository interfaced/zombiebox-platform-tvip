...
zb.device.platforms.tvip.ViewPort.prototype._setViewPort = function(screenArea) {
	...
	if (this._findRatioInAspectRatioMap(this._aspectRatio)) {
		this._plugin.player.setAspectRatio(
			/** @type {TVIPPlayer.Ratio} */(this._findRatioInAspectRatioMap(this._aspectRatio))
		);
	}
};
...
zb.device.platforms.tvip.ViewPort.prototype._initAspectRatioMap = function() {
	...
	this._aspectRatioMap['box'] = createAspectRatio(Proportion.AUTO, Transferring.AUTO);
	this._aspectRatioMap['full'] = createAspectRatio(Proportion.AUTO, Transferring.STRETCH);
	this._aspectRatioMap['zoom'] = createAspectRatio(Proportion.AUTO, Transferring.CROP);
};


/**
 * @param {zb.device.AspectRatio} ratio
 * @return {?TVIPPlayer.Ratio}
 * @protected
 */
zb.device.platforms.tvip.ViewPort.prototype._findRatioInAspectRatioMap = function(ratio) {
	var supportedMethods = /** @type {Array.<TVIPPlayer.Ratio>} */ (Object.keys(this._aspectRatioMap));

	return supportedMethods.filter(function(platformRatio) {
...
/**
 * @type {!Object.<TVIPPlayer.Ratio, zb.device.AspectRatio>}
 * @protected
 */
